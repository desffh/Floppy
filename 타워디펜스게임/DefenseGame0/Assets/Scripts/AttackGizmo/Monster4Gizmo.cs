...
public class Monster4Gizmo : MonoBehaviour
{
    ...
    private void Start()
    {
        Monster4Anime = GetComponent<Monster4Anime>();
    }
    ...
    void Update()
    {
        ...
        if (viewportPosition.x >= 0 && viewportPosition.x <= 1 && viewportPosition.y >= 0 && viewportPosition.y <= 1)
        {
            ...
            if (unitDetected != previousState)
            {
                if (unitDetected)
                {
                    Monster4Anime.StartAttack(); // 공격 애니메이션
                }
                else
                {
                    Monster4Anime.EndAttack(); // Idle 애니메이션
                }
